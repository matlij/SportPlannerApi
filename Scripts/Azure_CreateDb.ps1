...
New-AzSqlServer --location $location -ResourceGroupName $rg -ServerName $server -SqlAdministratorCredentials -SqlAdministratorCredentials $credentials
...
$connectionString= "Server=tcp:$server.database.windows.net,1433;Initial Catalog=$database;Persist Security Info=False;User ID=$adminSqlLogin;Password=$password;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;"
